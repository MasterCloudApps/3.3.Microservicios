package es.codeurjc.shop.orders.controller;
...
import es.codeurjc.shop.orders.exception.ResourceNotFoundException;
import es.codeurjc.shop.orders.repository.OrderRepository;
import es.codeurjc.shop.orders.saga.CreateOrderSaga;
...
class OrderController {
	...
	@Autowired
	private CreateOrderSaga orderSaga;
	...
	ResponseEntity<Order> createOrder(@RequestBody Order newOrder) throws JsonProcessingException {
		return ResponseEntity.status(HttpStatus.CREATED).body(orderSaga.createOrder(newOrder));
	}
